...
static int findbit(int low, int startscan, int pages, int memflags, int *len)
{
	...
	for (int i = 0; i < nbr_holes; i++) {
		if (pages <= holemap[i].len && holemap[i].len < best.len) {
			best.start = holemap[i].start;
			best.len = holemap[i].len;
		}
	}
 ...
 *				print_holes				     *
 *===========================================================================*/
void print_holes(int min_size)
{
	int nbr_holes = get_all_holes(0);

	printf("VM: Total number of holes: %d\n", nbr_holes);

	for (int i = 0; i < nbr_holes; i++) {
		if (holemap[i].len < min_size) { continue; }
		printf("VM: Hole %d [Page: %d, Size %d]\n", i, holemap[i].start, holemap[i].len);
	}
}
...
static int get_all_holes(int memflags)
{
	...
	if (memflags & PAF_LOWER16MB) { maxpage = boundary16 - 1; }
	else if (memflags & PAF_LOWER1MB) { maxpage = boundary1 - 1; }

	int freerange_start = maxpage;
	int run_length = 0, i;

	int nbr_holes = 0;
	memset(holemap, 0, sizeof(holemap));

	for (i = maxpage; i >= 0; i--) {
		if(!page_isfree(i)) {
			if (run_length) {
				holemap[nbr_holes].start = freerange_start;
				holemap[nbr_holes].len = run_length;
				nbr_holes++;
			}
