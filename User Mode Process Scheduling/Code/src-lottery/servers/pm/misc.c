 ...
 *				do_assigntickets			     *
 *===========================================================================*/
int do_assigntickets()
{
	int r, arg_which, arg_who, arg_tickets;
	struct mproc *rmp;

	arg_which = m_in.m1_i1;
	arg_who = m_in.m1_i2;
	arg_tickets = m_in.m1_i3;
	...
	if (mp->mp_effuid != rmp->mp_effuid && mp->mp_effuid != rmp->mp_realuid) {
		return EPERM;
	}

	if ((r = sched_assign_tickets(rmp, arg_tickets)) != OK) {
		return r;
	}
